...
# ═══ Conciliación bancaria RRHH ═══════════════════════════════════════════


@empleados_bp.post("/api/rrhh/conciliacion/ejecutar")
def api_rrhh_conciliacion_ejecutar():
  from core.rrhh_conciliacion import ejecutar_conciliacion
  data = request.get_json(silent=True) or {}
  ...
  try:
    result = ejecutar_conciliacion(periodo, tipos)
    return jsonify(result)
  except Exception as e:
    logger.exception("Error en conciliacion RRHH")
    return jsonify({"error": str(e)}), 500


@empleados_bp.get("/api/rrhh/conciliacion/<periodo>")
def api_rrhh_conciliacion_estado(periodo):
  from core.rrhh_conciliacion import obtener_estado_conciliacion
  return jsonify({"conciliaciones": obtener_estado_conciliacion(periodo)})


@empleados_bp.post("/api/rrhh/conciliacion/vincular")
def api_rrhh_conciliacion_vincular():
  from core.rrhh_conciliacion import vincular
  data = request.get_json(silent=True) or {}
  ok = vincular(data.get("conciliacion_id"), data.get("movimiento_banco_id"))
  return jsonify({"ok": ok})


@empleados_bp.post("/api/rrhh/conciliacion/desvincular")
def api_rrhh_conciliacion_desvincular():
  from core.rrhh_conciliacion import desvincular
  data = request.get_json(silent=True) or {}
  ok = desvincular(data.get("conciliacion_id"))
  return jsonify({"ok": ok})


@empleados_bp.get("/api/rrhh/conciliacion/buscar-movimientos/<int:cid>")
def api_rrhh_conciliacion_buscar(cid):
  from core.rrhh_conciliacion import buscar_movimientos_candidatos
  return jsonify({"movimientos": buscar_movimientos_candidatos(cid)})
